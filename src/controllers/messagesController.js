...
export const saveMessage = async ({ sender_id, recipient_id = null, group_id = null, content }) => {
  ...
  return withClient(async (client) => {
    ...
    // Validate group_id and membership for group messages
    if (group_id) {
    ...
    return normalizeMessage(rows[0], sender_id);
  });
};
...
export const updateMessage = async (req, res) => {
  ...
  try {
    const updatedMessage = await withClient(async (client) => {
      ...
      return rows.length ? normalizeMessage(rows[0], userId) : null;
    });
...
export const deleteMessageSocket = async (userId, messageId) => {
  return withClient(async (client) => {
    ...
    return normalizeMessage(deletedRows[0], userId);
  });
};
...
// ... (keep all existing imports and functions from previous responses)
...
// ... (keep all other existing functions: saveMessage, updateMessage, etc.)