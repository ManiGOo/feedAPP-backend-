  ...
  io.on("connection", (socket) => {
    ...
    socket.on("sendDM", async ({ to, content, tempId }) => {
      ...
      try {
        ...
        const client = await db.connect();
        try {
          const { rows } = await client.query(`SELECT username FROM users WHERE id = $1`, [socket.user.id]);
          const sender_username = rows[0]?.username || "User";

          const room = makeDMRoom(socket.user.id, to);
          io.to(room).emit("dmMessage", {
            id: newMessage.id,
            ...
            isOwn: newMessage.sender_id === socket.user.id,
            tempId,
            sender_username,
          });
        } finally {
    ...
    socket.on("createGroup", async ({ id, name, members, role }) => {
      if (!id || !name?.trim() || !Array.isArray(members) || members.length === 0) {
        return socket.emit("errorMessage", { error: "Group ID, name, and members required" });
      }
      try {
        const client = await db.connect();
        try {
          ...
          if (rows.length === 0) {
            return socket.emit("errorMessage", { error: "Group does not exist" });
          }
          ...
          if (validMemberIds.length !== members.length) {
            return socket.emit("errorMessage", { error: "One or more member IDs are invalid" });
          }
          ...
          for (const memberId of members) {
            io.to(`user_${memberId}`).emit("groupCreated", {
              id,
              name,
              role: memberId === socket.user.id ? role : "member",
              members,
            });
            // Ensure member is in their user room
            io.sockets.sockets.forEach((s) => {
              if (s.user.id === memberId) {
                s.join(`user_${memberId}`);
              }
            });
          }
          console.log(`✅ Group created: ${name} (id=${id})`);
        } finally {
          client.release();
        }
      } catch (err) {
        console.error("❌ Error creating group:", err.message);
        socket.emit("errorMessage", { error: err.message || "Failed to create group" });
      }
    });
